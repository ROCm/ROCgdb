...
//-- AMD CHANGE
#ifdef DEBUG_MSVC_DEMANGLE
#define IF_DEBUG_MSVC_DEMANGLE(x) x
#else
#define IF_DEBUG_MSVC_DEMANGLE(x)
#endif

#define SET_ERROR()                      \
  do {                                   \
    IF_DEBUG_MSVC_DEMANGLE(              \
      printf("Error @ %d: func: '%s',"   \
	     " file %s!\n",              \
	     __LINE__, __func__,         \
	     __FILE__));                 \
    Error = true;                        \
...
#if 0 // removed not to trigger any compiler defined symbols
...
Demangler::demangleVcallThunkNode(std::string_view &MangledName) {
  ...
  if (!Error) {
    bool err = !consumeFront(MangledName, "$B");
    SET_ERROR_C(err);
  }
  if (!Error)
