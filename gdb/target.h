...
struct target_ops
  : public refcounted_object
  {
    ...
    /* Return true if the target has pending events to report to the
       ...
       FALSE.  The only downside is that a potential optimization is
       missed.  */
...
/* Return true if the target has events to report to the core.  See
   target_ops::has_events().  */

static inline bool
target_has_events ()
{
  return current_top_target ()->has_events ();
}
