...
wave_coordinates::fetch ()
{
  ...
  if (cluster_ids[0] != UINT32_MAX)
    {
      /* Group ids are the coordinates within the grid.
	 Convert them to coordinates within the cluster.  */
      vec3_u32_t cluster_sizes;
      dispatch_get_info_throw (dispatch_id,
			       AMD_DBGAPI_DISPATCH_INFO_CLUSTER_SIZES,
			       cluster_sizes);

      vec3_t<uint16_t> wg_sizes;
      dispatch_get_info_throw (dispatch_id,
			       AMD_DBGAPI_DISPATCH_INFO_WORKGROUP_SIZES,
			       wg_sizes);

      group_ids[0] = group_ids[0] % (cluster_sizes[0] / wg_sizes[0]);
      group_ids[1] = group_ids[1] % (cluster_sizes[1] / wg_sizes[1]);
      group_ids[2] = group_ids[2] % (cluster_sizes[2] / wg_sizes[2]);
    }
