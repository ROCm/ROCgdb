...
rw_closure_value (value *v, value *from)
{
  ...
  frame_info_ptr frame = VALUE_CONTEXT (v).frame ();
...
{
  LONGEST total_bit_offset = value_offset (value) * HOST_CHAR_BIT;
				  ...
				  LONGEST subobj_offset, bool as_lval)
{
  ...
  if (frame_id_p (VALUE_CONTEXT (result).next_frame_id))
    this->m_context.next_frame_id = VALUE_CONTEXT (result).next_frame_id;
  VALUE_CONTEXT (result) = this->m_context;
  return result;
}
