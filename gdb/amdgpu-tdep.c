			...
			arch_addr_space_id to_aspace_id)
{
  ...
  /* Get the dbgapi address space id for the destination address space.  */
...
/* Form a core address from a TYPE pointer type information and BUF
   ...
   information.  If available, we use the address space information.
   If empty, it is possible that TYPE has address class (i.e.,
   DW_AT_address_class) information.  OpenCL currently encodes source
   language address space qualifiers in DW_AT_address_class.
   Therefore we cannot ignore address class and we handle them as
   "best effort".  Based on the direction DWARF 6 is taking, it is
   expected that OpenCL would stop using DW_AT_address_class and use a
   new attribute instead.  When that happens, all address-class
   related code, including gdbarch methods, can be removed.  We just
   need address space.  */
static CORE_ADDR
