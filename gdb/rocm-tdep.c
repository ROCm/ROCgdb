...
static const struct inferior_data *rocm_inferior_data_key;
...
get_rocm_inferior_info (struct inferior *inferior)
{
  ...
  struct rocm_inferior_info *info = static_cast<struct rocm_inferior_info *> (
      inferior_data (inferior, rocm_inferior_data_key));
  if (!info)
    {
      ...
      info->breakpoint_map
          = new std::unordered_map<decltype (amd_dbgapi_breakpoint_id_t::handle),
                     struct breakpoint *>;
      info->wave_stop_events = new std::list<
          std::pair<amd_dbgapi_wave_id_t, amd_dbgapi_wave_stop_reason_t>>;
      info->notify_solib_map = new std::unordered_map<
          decltype (amd_dbgapi_shared_library_id_t::handle),
          struct rocm_notify_shared_library_info>;

      set_inferior_data (inferior, rocm_inferior_data_key, info);
    }
...
rocm_breakpoint_check_status (struct bpstats *bs)
{
  ...
  auto it = std::find_if (
      ...
      [=] (const std::decay<decltype (*info->breakpoint_map)>::type::value_type &value) {
  ...
  if (msymbol.minsym != NULL)
    {
      ...
      xsnprintf (buf, sizeof (buf), "%s", MSYMBOL_PRINT_NAME (msymbol.minsym));
...
rocm_target_inferior_exit (struct inferior *inf)
{
  ...
  if (info->breakpoint_map)
    {
      /* Delete the breakpoints that are still active.  */
      for (auto &&value : *info->breakpoint_map)
        delete_breakpoint (value.second);
      info->breakpoint_map->clear ();
    }

  if (info->wave_stop_events)
    info->wave_stop_events->clear ();

  if (info->notify_solib_map)
    info->notify_solib_map->clear ();
}

/* Handles the cleanup of the per-inferior rocm_inferior_info data.  */

static void
rocm_inferior_cleanup (struct inferior *inf, void *arg)
{
  struct rocm_inferior_info *info
      = static_cast<struct rocm_inferior_info *> (arg);
  ...
  if (info->breakpoint_map)
    {
      ...
      for (auto &&value : *info->breakpoint_map)
        delete_breakpoint (value.second);
      delete info->breakpoint_map;
    }

  if (info->wave_stop_events)
    delete info->wave_stop_events;

  if (info->notify_solib_map)
    delete info->notify_solib_map;

  xfree (info);
}

static cli_style_option warning_style (ui_file_style::RED);
static cli_style_option info_style (ui_file_style::GREEN);
static cli_style_option verbose_style (ui_file_style::BLUE);
  ...
  for (size_t i = 0; i < count; ++i)
    {
      ...
      /* agent  */
      uiout->field_int ("agent_id", agent_list[i].handle);
...
_initialize_rocm_tdep (void)
{
  ...
  rocm_inferior_data_key
      = register_inferior_data_with_cleanup (nullptr, rocm_inferior_cleanup);
