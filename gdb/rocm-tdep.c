          ...
          for (auto &&agent_id : agents)
            {
              ...
              if ((status = amd_dbgapi_agent_get_info (
                       agent_id, AMD_DBGAPI_AGENT_INFO_NAME,
...
static void
show_queue (const char *args, int from_tty)
{
  cmd_show_list (queue_list, from_tty, "");
}
...
static void
show_dispatch (const char *args, int from_tty)
{
  cmd_show_list (dispatch_list, from_tty, "");
}
...
Otherwise, all agents are displayed."),
  ...
  add_prefix_cmd ("queue", class_run, show_queue,
...
Otherwise, all queues are displayed."),
  ...
  add_prefix_cmd (
      "dispatch", class_run, show_dispatch,
