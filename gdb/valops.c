		     ...
		     int subclass_check)
{
  ...
  if (aspace1 != aspace2
    ...
    {
      /* This happens when, for example, there is a pointer PTR to
	 generic address space and the user does
	...
	if (toval->bitsize ())
	...
	else if (toval->bitpos ())
	  {
	    ...
	    copy_bitwise (buffer.data (), bitpos, fromval->contents ().data (),
			  0, type->length () * HOST_CHAR_BIT, big_endian);
	    dest_buffer = buffer.data();
	  }
	else
	  {
...
value_repeat (struct value *arg1, int count)
{
  ...
  value *val
    = allocate_repeat_value (arg1->enclosing_type (), count);
