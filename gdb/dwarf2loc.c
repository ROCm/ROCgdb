...
class dwarf_evaluate_loc_desc : public dwarf_expr_context
{
 public:
  ...
  {
    return indirect_synthetic_pointer (die_offset, offset, 
                                       per_cu, frame, type);
  }
};
...
dwarf2_evaluate_loc_desc (struct type *type, struct frame_info *frame,
			  const gdb_byte *data, size_t size,
			  struct dwarf2_per_cu_data *per_cu, bool as_lval)
{
  return dwarf2_evaluate_loc_desc_full (type, frame, data, size,
					 per_cu, NULL, 0, as_lval);
}
