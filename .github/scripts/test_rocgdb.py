...
# --emulator mode when --emulator-test-timeout is not provided.
EMULATOR_DEFAULT_WALL_CLOCK_TIMEOUT = 3600
...
def _run_command(
    ...
    if kill_process_group:
        ...
        try:
            ...
            # runtest / expect / gdb process group is never orphaned.
            try:
...
def parse_arguments() -> argparse.Namespace:
  ...
  python %(prog)s --emulator --tests gdb.rocm/foo.exp
    ...
    parser.add_argument(
        ...
        "If --parallel is used without -j, unlimited jobs are used (make -j).",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "under --emulator-log-dir. Mutually exclusive with --parallel.",
    )
    parser.add_argument(
        "--emulator-log-dir",
        type=Path,
        default=None,
        metavar="PATH",
        help="Root directory for per-test gdb.log captures in --emulator mode. "
        "Default: <testsuite_dir>/emulator_logs.",
    )
    parser.add_argument(
        ...
        "in --emulator mode. Defaults to 3600.",
    )
    ...
    # Validate --emulator constraints.
    if args.emulator and args.parallel:
        _log_error_and_exit("--emulator cannot be combined with --parallel.")
    if (
        args.emulator_log_dir is not None or args.emulator_test_timeout is not None
    ) and not args.emulator:
        _log_error_and_exit(
            "--emulator-log-dir / --emulator-test-timeout require --emulator."
        )
...
def _log_aligned_fields(
    ...
    """
    Log (label, value) pairs with colons aligned to the longest label.

    Rows where `value` is None are logged as plain `{indent}{label}`,
    skipping the colon and padding — useful for pass-through lines mixed
    in with key/value rows.

    Args:
        fields: List of (label, value) tuples; value=None marks a pass-through line.
    ...
    if not fields:
        return
    labelled = [label for label, value in fields if value is not None]
...
def discover_test_directories(
    ...
    for item in testsuite_dir.iterdir():
        if item.is_dir() and item.name.startswith("gdb.") and item.name not in exclude:
            # Check if directory contains any .exp files.
            exp_files = list(item.glob("*.exp"))
            if exp_files:
...
def run_tests(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if args.emulator:
            if args.emulator_test_timeout is not None:
                wall_clock = args.emulator_test_timeout
            else:
                # Emulator targets are slow and gdb_test_timeout is
                # per-testcase, not per-.exp. Default generously and let the
                # user override via --emulator-test-timeout.
                wall_clock = EMULATOR_DEFAULT_WALL_CLOCK_TIMEOUT
                logger.info(
                    f"Using default emulator wall-clock timeout: {wall_clock}s "
                    f"(override with --emulator-test-timeout)"
                )
            log_dir = args.emulator_log_dir or (test_suite_dir / "emulator_logs")
            _execute_one_by_one(
                test_suite_dir,
                current_tests,
                runtestflags_str,
            ...
            if args.parallel:
                if args.jobs is not None:
                    cmd += ["FORCE_PARALLEL=1", f"-j{args.jobs}"]
                else:
        ...
        if not failed_tests:
            ...
            # Skipped in --emulator mode since per-test logs are already on disk.
            if args.dump_failed_test_log and not args.emulator:
...
def _execute_one_by_one(
    ...
    base_suffix = "" if iteration == 1 else f".retry-{iteration - 1}"
    ...
    for test in tests:
        ...
        logger.info(f"[emulator] {compiler_label} - {test}: {shlex.join(cmd)}")
        ...
        if gdb_sum.is_file():
            per_test_lines = _read_file_lines(
                gdb_sum, f"during emulator aggregation of {test}"
            )
            ...
            per_test_lines.append(
                f"UNRESOLVED: {test}: emulator make produced no gdb.sum"
            )
        ...
        if wall_clock_timed_out:
            per_test_lines.append(
                f"FAIL: {test}: emulator wall-clock timeout (timeout)"
            )
        ...
        has_error = any(line.startswith("ERROR:") for line in per_test_lines)
        has_unresolved = any(line.startswith("UNRESOLVED:") for line in per_test_lines)
        has_fail = any(line.startswith("FAIL:") for line in per_test_lines)
        ...
        status_content = (
            ...
            f"wall_clock_timeout: {'true' if wall_clock_timed_out else 'false'}\n"
        )
        try:
...
def print_configuration(
    ...
    Args:
        ...
        ignore_list_file_status (str):
            Status message for ignore list file.
        output_ignore_list_file_status (str):
            Status message for output ignore list file.
    ...
    if not args.parallel:
        ...
        parallel_info = "Enabled (unlimited)"
    else:
    ...
    emulator_timeout_display = (
        f"{args.emulator_test_timeout} seconds"
        if args.emulator_test_timeout is not None
        else f"{EMULATOR_DEFAULT_WALL_CLOCK_TIMEOUT} seconds (auto)"
    )
    ...
    fields = [
        ...
        ("Emulator Mode", "Enabled" if args.emulator else "Disabled"),
    ]
    if args.emulator:
        fields.append(
            (
                "Emulator Log Dir",
                args.emulator_log_dir or (testsuite_dir / "emulator_logs"),
            )
        )
        fields.append(("Emulator Test Timeout", emulator_timeout_display))
    fields.extend(
        [
