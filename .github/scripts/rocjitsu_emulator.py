...
# Emulated tests run far slower than on hardware, so dejagnu's default
# per-testcase timeout is what makes the slow ones fail rather than any real
# problem.
DEFAULT_TIMEOUT = 600
...
def normalize_profile(spec: str) -> Tuple[str, str]:
    ...
    candidates = [key] + ([f"{key}x"] if key[-1:].isdigit() else [])
...
def parallel_jobs(
    ...
    if available_gb is not None:
        fits = max(0, int((available_gb - BASE_MEMORY_GB) / JOB_MEMORY_GB))

    # Even one job needs BASE_MEMORY_GB + JOB_MEMORY_GB. Below that (fits == 0)
    # the OOM killer is likely to take tests down whatever the job count, so the
    # warning has to fire even when the caller pinned --jobs 1.
    below_one_job = fits == 0

    if requested is not None:
        if below_one_job:
            return JobAdvice(
                requested,
                f"Only {available_gb:.0f}GB is available, below the roughly "
                f"{BASE_MEMORY_GB + JOB_MEMORY_GB:.0f}GB even one job needs: "
                f"the OOM killer will take tests down, which reads as failures.",
                True,
            )
    ...
    # Start from the CPU/ceiling cap; the memory cap just below overrides both
    # the count and this reason whenever memory is the tighter limit.
    reason = (
        f"the {JOB_CEILING} job ceiling" if jobs == JOB_CEILING else "this host's CPUs"
    )
    if fits is not None and fits < jobs:
        # Memory is the tighter limit. Keep at least one job running; the
        # below_one_job flag carries the warning if even that will not fit.
        jobs, reason = max(1, fits), f"{available_gb:.0f}GB of available memory"
    message = (
        ...
        f"wall clock, so pass --jobs to override."
    )
    if below_one_job:
        message += (
            f" Only {available_gb:.0f}GB is available, below the roughly "
            f"{BASE_MEMORY_GB + JOB_MEMORY_GB:.0f}GB one job needs, so expect "
            f"OOM-killed tests."
        )
    return JobAdvice(jobs, message, below_one_job)
...
def check_profile_arch(rocm_root: Path, profile: str, arch: str) -> None:
    ...
    try:
        loaded = json.loads(info.read_text()).get("dist_amdgpu_targets", "")
    except (OSError, ValueError, AttributeError):
        return
    # TheRock writes this as a comma/space/semicolon separated string; anything
    # else is a dist file we do not understand, so leave it alone.
...
class Emulator:
    ...
    def wrap(self, cmd: Sequence[str]) -> List[str]:
        ...
        environment on to the command, so nothing extra needs setting here.
        """
    ...
    def runtest_wrapper(self) -> Path:
        ...
        # self.profile is embedded in the generated script. normalize_profile()
        # validated it against PROFILE_ARCH, so no unchecked input reaches here.
        ...
        with os.fdopen(
            ...
            out.write(
                ...
                # GDB_LOCK_DIR=... is a dejagnu command-line assignment consumed
                # by runtest, which reads the Tcl global $::GDB_LOCK_DIR in
                # lock_dir(). It is not a positional argument, so do not rewrite
                # it as a --env variable.
    ...
    def cleanup(self) -> None:
        ...
        sessions alone, so calling it mid-run is safe. A failure here only means
        a stranded emulated GPU costing memory, so warn rather than fail.
        """
        try:
            done = subprocess.run(
            ...
            logger.warning("mirage cleanup failed: %s", e)
            return
