...
def parallel_jobs(
    ...
    # Below one job's worth of memory even a single job is likely OOM-killed, so
    # flag it rather than reporting a clean one-job run.
    low_memory = (
        available_gb is not None and available_gb < BASE_MEMORY_GB + JOB_MEMORY_GB
    )
    if low_memory:
        message += (
            f" Only {available_gb:.0f}GB is available, below the roughly "
            f"{BASE_MEMORY_GB + JOB_MEMORY_GB:.0f}GB one job needs; expect "
            f"OOM-killed tests."
        )
    return JobAdvice(jobs, message, low_memory)
